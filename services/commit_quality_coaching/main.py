...
from shared.models import (
    ...
    CommitQuality, AnalysisStatus, BehaviorPatternType
)
...
class CoachingFeedback(BaseModel):
    ...
    specific_recommendations: List[str] = Field(default_factory=list, description="Specific recommendations")
...
class CoachingSession(BaseModel):
    ...
    commits_reviewed: List[str] = Field(default_factory=list, description="Commits reviewed in session")
...
class UserProgress(BaseModel):
    ...
    regression_areas: List[str] = Field(default_factory=list, description="Areas showing regression")
...
class CommitQualityCoachingService:
    ...
    def _analyze_message_quality(self, message: str) -> float:
        ...
        descriptive_words = ['because', 'when', 'where', 'why', 'how', 'fixes', 'closes', 'implements']
    ...
    def _get_message_analysis(self, message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_imperative": message.lower().startswith(('add', 'fix', 'update', 'remove', 'refactor', 'improve')) if message else False,
            "has_issue_reference": any(char in message for char in ['#', 'GH-', 'JIRA-']) if message else False,
            "is_descriptive": any(word in message.lower() for word in ['because', 'when', 'where', 'why', 'how', 'fixes', 'closes', 'implements']) if message else False
        }
    ...
    def _get_file_analysis(self, changed_files: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_extensions": len(set(file_extensions)),
            "extension_distribution": {ext: file_extensions.count(ext) for ext in set(file_extensions)}
        }
    ...
    def _get_size_analysis(self, changed_files: List[str]) -> Dict[str, Any]:
        ...
        return {
            "file_count": len(changed_files),
            "size_category": "small" if len(changed_files) <= 3 else "medium" if len(changed_files) <= 10 else "large"
        }
    
    async def generate_coaching_feedback(self, commit: Commit, analysis: Dict[str, Any]) -> CoachingFeedback:
    ...
    async def get_user_progress(self, user_id: str, days: int = 30, db: AsyncSession = None) -> UserProgress:
