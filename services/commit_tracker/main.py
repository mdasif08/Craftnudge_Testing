...
from shared.database import DatabaseService, get_database_health
...
class CommitTrackerService:
    ...
    async def track_recent_commits(
        self, repo_path: str, days: int = 7, limit: int = 100
    ) -> List[Commit]:
    ...
    async def publish_commit_event(self, commit: Commit):
        ...
        try:
            ...
            event = self.event_factory.create_commit_event(
                commit_data, source=EventSource.COMMIT_TRACKER, correlation_id=commit.id
            )
            ...
            # Publish to Redis
            await self.redis_client.publish("commit_events", EventSerializer.serialize(event))
...
class TrackCommitRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"repository_path": "/path/to/repo", "commit_hash": "abc123def456"}
        }
...
class TrackRecentCommitsRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"repository_path": "/path/to/repo", "days": 7, "limit": 100}
        }
...
async def startup_event():
    ...
    try:
        ...
        logger.warning(f"Failed to initialize service: {e}. Service will start without Redis connection.")
...
async def track_commit(request: TrackCommitRequest, background_tasks: BackgroundTasks):
    """Track a specific commit or the latest commit."""
    try:
        commit = await commit_tracker_service.track_commit(
            request.repository_path, request.commit_hash
        )
...
async def track_recent_commits(request: TrackRecentCommitsRequest):
    ...
    try:
        commits = await commit_tracker_service.track_recent_commits(
            request.repository_path, request.days, request.limit
        )
...
async def get_commit_statistics(repository_path: str):
    ...
    try:
        ...
        return CommitStatisticsResponse(repository_path=repository_path, statistics=statistics)
...
async def main():
    ...
    try:
        ...
        if args.recent:
            ...
            commit = await commit_tracker_service.track_commit(args.repo_path, args.commit_hash)
