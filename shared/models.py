...
class Commit(CommitBase):
    ...
    quality_score: Optional[float] = Field(
        None, ge=0, le=10, description="AI-assessed quality score"
    )
...
class AnalysisModel(Base):
    ...
    ai_model = Column(String(100), nullable=False, default="llama2")
    ...
    __table_args__ = (
        ...
        Index("idx_analyses_ai_model", "ai_model"),
    )
...
class BehaviorPatternResponse(BaseModel):
    """Response model for BehaviorPattern."""
...
class UserBehaviorCreate(BaseModel):
    """Create model for UserBehavior."""

    user_id: str = Field(..., description="User ID")
    commit_id: str = Field(..., description="Commit ID")
    behavior_type: str = Field(..., description="Behavior type")
...
class UserBehaviorUpdate(BaseModel):
    """Update model for UserBehavior."""

    behavior_type: Optional[str] = Field(None, description="Behavior type")
    description: Optional[str] = Field(None, description="Behavior description")
    confidence: Optional[float] = Field(None, ge=0, le=1, description="Confidence score")
    impact_score: Optional[float] = Field(None, ge=0, le=10, description="Impact score")


class UserBehaviorResponse(BaseModel):
    """Response model for UserBehavior."""
...
DatabaseCommit = CommitModel
DatabaseAnalysis = AnalysisModel
DatabaseBehaviorPattern = BehaviorPatternModel
DatabaseUserBehavior = UserBehaviorModel
