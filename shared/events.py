...
class Event(BaseEvent):
    """Main event class for the event system."""

    def __init__(self, **data):
        super().__init__(**data)

    @classmethod
    def create(cls, event_type: EventType, data: Dict[str, Any], **kwargs) -> "Event":
        """Create an event."""
...
class AnalysisData(BaseModel):
    ...
    behavioral_insights: Dict[str, Any] = Field(
        default_factory=dict, description="Behavioral insights"
    )
    analysis_duration: float = Field(default=0.0, description="Analysis duration in seconds")
    ai_model: str = Field(default="llama2", description="AI model used")
...
class EventFactory:
    ...
    def create_event(
        ...
        **kwargs,
        ...
        return BaseEvent(metadata=metadata, event_type=event_type, data=data)

    @staticmethod
    def create_commit_event(
        commit_data: CommitData, event_type: EventType = EventType.COMMIT_CREATED, **kwargs
    ) -> CommitEvent:
    ...
    def create_analysis_event(
        analysis_data: AnalysisData, event_type: EventType = EventType.ANALYSIS_COMPLETED, **kwargs
    ) -> AnalysisEvent:
        """Create an analysis event."""
        return AnalysisEvent.create(event_type, analysis_data, **kwargs)

    @staticmethod
    def create_error_event(error_data: ErrorData, **kwargs) -> ErrorEvent:
        """Create an error event."""
...
class EventUtils:
    ...
    def is_retryable_event(event: BaseEvent) -> bool:
        ...
        return (
            ...
            and event.metadata.status != EventStatus.COMPLETED
        )
    ...
    def filter_events_by_time_range(
        events: List[BaseEvent], start_time: datetime, end_time: datetime
    ) -> List[BaseEvent]:
        """Filter events by time range."""
        return [event for event in events if start_time <= event.metadata.timestamp <= end_time]
