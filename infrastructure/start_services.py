    ...
    def __init__(self):
        ...
        self.services = {
            "commit-tracker": {
                ...
                "health_url": f"http://localhost:{settings.services.commit_tracker.port}/health",
                "command": ["python", "services/commit_tracker/run.py"],
                "process": None,
                "status": "stopped",
            },
            "commit-quality-coaching": {
                ...
                "health_url": f"http://localhost:{settings.services.commit_quality_coaching.port}/health",
                "command": ["python", "services/commit_quality_coaching/run.py"],
                "process": None,
                "status": "stopped",
            },
        }
    ...
    async def start_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            process = subprocess.Popen(
                service["command"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
                ...
                try:
                    async with httpx.AsyncClient(timeout=5.0) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            self.console.print(
                                f"[green]✅ {service['name']} started successfully[/green]"
                            )
                            return True
                except Exception:
                    pass
    ...
    async def start_all_services(self) -> bool:
        ...
        with Progress(
            ...
            for service_name in startup_order:
                progress.update(
                    task, description=f"Starting {self.services[service_name]['name']}..."
                )

                success = await self.start_service(service_name)
                if not success:
                    self.console.print(
                        f"[red]❌ Failed to start {service_name}, stopping all services[/red]"
                    )
    ...
    async def stop_all_services(self) -> bool:
        ...
        with Progress(
            ...
            for service_name in self.services:
                progress.update(
                    task, description=f"Stopping {self.services[service_name]['name']}..."
                )
    ...
    async def monitor_services(self, interval: int = 30):
        ...
        while self.running:
            ...
            for service_name, health in health_status.items():
                status_style = {"healthy": "green", "unhealthy": "red", "error": "red"}.get(
    ...
    async def main():
        ...
        if check_infrastructure:
            if not await service_manager.check_infrastructure():
                console.print(
                    "[red]❌ Infrastructure check failed. Please start Redis, PostgreSQL, and Ollama first.[/red]"
                )
                return
...
def health():
    ...
    async def main():
        ...
        for service_name, health in health_status.items():
            ...
            response_time = (
                f"{health.get('response_time', 0):.3f}s" if health.get("response_time") else "N/A"
            )
            details = health.get("error", "OK")
